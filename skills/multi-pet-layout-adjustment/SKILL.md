...
**实现与脚本**：本技能逻辑在 `scripts/run_pet_layout_adjustment.py`、`scripts/state_manager.py` 与 `scripts/run_multi_pet_composition.py`；此处为项目内 Skill 文档，便于与编排器及其他技能配合。
...
| 输出 | `design_path` | — | 重新合成后的设计图路径（如 sessions/<session_id>/design.png） |
...
5. **输出**：合成结果保存到 `sessions/<session_id>/design.png`，返回该路径。
